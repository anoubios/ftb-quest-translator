...
public class QuestTranslationManager {
    ...
    public void checkConnectionInBackground() {
        ...
        if (isCheckingConnection.compareAndSet(false, true)) {
            if (FabricConfig.getTranslationProvider() == FabricConfig.TranslationProvider.AUTO) {
    ...
    private FabricConfig.TranslationProvider lastProviderEnum = null;
    private String lastDeepLApiKey = null;

    private TranslationProvider getProvider() {
        FabricConfig.TranslationProvider provider = FabricConfig.getTranslationProvider();
        ...
        if (provider == FabricConfig.TranslationProvider.AUTO) return getAutoProvider();
        if (provider == FabricConfig.TranslationProvider.MYMEMORY) return myMemoryProvider;
        if (provider == FabricConfig.TranslationProvider.DEEPL) return deepLProvider;
    ...
    public String getActiveAutoProviderName() {
        if (FabricConfig.getTranslationProvider() != FabricConfig.TranslationProvider.AUTO) return getProvider().getName();
    ...
    public void toggleTranslation(long questId, String title, String subtitle, List<String> description, Runnable onComplete) {
        ...
        String targetLang = getTargetLanguage(); TranslationProvider provider = getProvider(); TranslationCache cache = TranslationCache.getInstance(); boolean useCache = FabricConfig.isEnableCaching();
