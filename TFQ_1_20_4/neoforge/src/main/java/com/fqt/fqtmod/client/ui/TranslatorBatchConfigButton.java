...
public class TranslatorBatchConfigButton extends TabButton {
    ...
    public void onClicked(MouseButton button) {
        ...
        if (!mgr.isRunning() && mgr.getProgress() != null && !mgr.getProgress().pendingQuestIds.isEmpty()) {
            List<ContextMenuItem> items = new ArrayList<>();
            items.add(new ContextMenuItem(Component.translatable("ftbquesttransl.batch.continue_old", mgr.getCompletedQuests(), mgr.getTotalQuests()), Icons.ACCEPT, b -> {
                mgr.resumeBatch();
            }));
            items.add(new ContextMenuItem(Component.translatable("ftbquesttransl.batch.start_new"), Icons.ADD, b -> {
